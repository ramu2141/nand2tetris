...
class CompilationEngine():
    ...
    def identifier(self, name:str, type:str='', cat:str='', defined:bool=False):
        '''
        構造
        <identifier>
        <category> ( class | subroutine | var | argument | static | field ) <category>
        <name></name>
        <type></type>
        <kind> ( var | argument | static | field ) </kind>
        <index></index>
        ( <defined/> | <used/> )
        </identifier>
        '''

        self.xmlf.write('<identifier>\n')

        if defined == True:
            self.xmlf.write(f'<category> {cat} </category>\n<name> {name} </name>\n')
            if cat in ('class', 'subroutine'):
                pass
            else:
                self.symtbl.define(name, type, cat)
                index = self.symtbl.index_of(name)
                self.xmlf.write(f'<type> {type} </type>\n<kind> {cat} </kind>\n<index> {index} </index>\n')
            self.xmlf.write('<defined/>\n</identifier>\n')
        else:
            if cat in ('class', 'subroutine'):
                self.xmlf.write(f'<category> {cat} </category>\n<name> {name} </name>\n<used/>\n</identifier>\n')
            else:
                kind = self.symtbl.kind_of(name)
                if kind is None:
                    self.xmlf.write(f'<category> class </category>\n<name> {name} </name>\n<used/>\n</identifier>\n')
                else:
                    type_ = self.symtbl.type_of(name)
                    index_ = self.symtbl.index_of(name)
                    self.xmlf.write(f'<category> {kind} </category>\n<name> {name} </name>\n')
                    self.xmlf.write(f'<type> {type_} </type>\n<kind> {kind} </kind>\n')
                    self.xmlf.write(f'<index> {index_} </index>\n<used/>\n</identifier>\n')
        return None
    ...
    def compile_class(self):
        ...
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier(), '', 'class', True)
            self.advance()
        else:
    ...
    def compile_class_var_dec(self):
        ...
        # varName
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier(), type, kind, True)
            self.advance()
        else:
    ...
    def compile_subroutine(self):
        ...
        # subroutineName
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier(), '', 'subroutine', True)
            self.advance()
        else:
    ...
    def compile_parameter_list(self):
        ...
        # varName
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier(), type, 'argument', True)
            self.advance()
        else:
    ...
    def compile_var_dec(self):
        ...
        # varName
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier(), type, 'var', True)
            self.advance()
        else:
    ...
    def compile_do(self):
        ...
        if self.jt.token_type() == 'SYMBOL' and self.jt.symbol() == '(':
            ...
            # subroutineName
            self.identifier(ident, '', 'subroutine')
            ...
            # subroutineName
            if self.jt.token_type() == 'IDENTIFIER':
                self.identifier(self.jt.identifier(), '', 'subroutine')
                self.advance()
            else:
    ...
    def compile_let(self):
        ...
        # varName
        if self.jt.token_type() == 'IDENTIFIER':
            self.identifier(self.jt.identifier())
            self.advance()
        else:
